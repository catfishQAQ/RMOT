...
class PhyPGDAttackerFF:
    def __init__(
        ...
        if self.attack_type == "phy_ff":
            self.loss_weights = loss_weights
        else:
            raise ValueError(f"Unknown mode: {attack_type}. Supported modes: 'phy_ff'.")
    ...
    def perturb_aai(self, fid: int, img: torch.Tensor, img_hw: tuple, track_instances: Optional[Instances] = None):
        ...
        div = 30 # # Mild: [5, 10], Moderate: [10, 20], and Severe: [20, 30]
        ...
        for step in range(self.steps):
            ...
            loss = - (loss_ce + loss_conf + loss_bbox + loss_giou) 
