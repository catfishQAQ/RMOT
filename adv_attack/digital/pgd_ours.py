...
class PGDAttackerOurs:
    def __init__(
        ...
        alpha: float = 1 / 255,
        steps: int = 40,
        attack_type: str = "ours_1", # "ff", "hijack", "dedalus", "ours_1", "ours_2"
        ...
        if attack_type == "ours_1" or attack_type == "ours_2":
            self.loss_weights = loss_weights
        else:
            raise ValueError(f"Unknown mode: {attack_type}. Supported modes: 'ours_1', 'ours_2'.")
    ...
    def perturb(self, fid: int, img: torch.Tensor, img_hw: tuple, track_instances: Optional[Instances] = None):
        ...
        for step in range(self.steps):
            ...
            if self.attack_type == "ours_1":
                loss =  loss_matching 
            elif self.attack_type == "ours_2":
                loss = - loss_reborn 
            else:
                raise ValueError(f"Unknown mode: {self.mode}. Supported modes: 'ours_1', 'ours_2'.")
